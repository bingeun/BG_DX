...
#pragma warning(pop)
/*! \sa  Class ReferenceTarget, Class IParamArray, ~{ %Parameter Maps }~,
...
class IParamBlock : 			
	...
	public:
		...
		it is FALSE.\n\n
		\sa
		~{ %Parameter Blocks }~in the Advanced Topics section,
		~{ Intervals }~.
		*/
...
/*! \brief This is a handy post load call back for fixing up parameter blocks. 
	This will look up the version of the loaded callback and fix it up so it matches the
	current version. NOTE: this deletes itself when its done. See
	~{ Parameter Maps }~
	for more details. \sa  Class ParamVersionDesc. */
...
protected:
	...
	int pbRefNum;
	AnimHandle refHandle;
	CoreExport void process(ILoad* ILoad);
public:
	/*! \brief Constructor.
	\param versions - This is an array of ParamVersionDescs.
	\param count - The number of elements in the array specified above.
	\param current - This is a pointer to the current version of the ParamVersionDesc.
	\param target - This is a pointer to a reference target. This is usually the this pointer of the object.
	\param refNum - This is the reference index of the parameter block.
	\sa  Advanced Topics section under
	~{ %Parameter Maps }~ for an explanation of how this is used. */
	CoreExport ParamBlockPLCB(ParamVersionDesc* versions ,
								int               count    ,
								ParamVersionDesc* current  ,
								ReferenceTarget*  target   ,
								int               refNum);
	CoreExport ~ParamBlockPLCB();
	...
	This performs extra checks to ensure that ReferenceTarget pointer
	targ is still valid and hasn't been deleted.
	\return Returns targ if the ReferenceTarget pointer is still valid, otherwise NULL. */
	CoreExport ReferenceTarget* GetTarget();
	/*! Gets the refNum Reference on the member targ. */
	CoreExport IParamBlock* GetParamBlockRef();

	/* Method to ensure that the ReferenceTarget and the Parameter block
	held by this post load callback are still valid and not deleted. 
	Specifically it checks that the ReferenceTarget has not been deleted.
	It checks that pbRefNum is greater than or equal to zero and less 
	the number of references on the target. It also checks that the 
	super class id is actually PARAMETER_BLOCK_CLASS_ID. And last of all
	it attempts to cast the reference held by targ at pbRefNum to a 
	Parameter block. 
