...
class Array;
namespace MaxSDK { namespace Util { class Char; } }
...
public:
	...
	bool		in_attributes_def;	// compiling attributes definition, resource values captured to source_capture
									// instead of resource ids since saved to scene file
	...
	MaxSDK::Array<ParserSourceData> src_parser_data_stack;  // include source position data stack	
...
/* lexical analysis methods */
	...
	void		add_tool_local(const MCHAR* var, MouseTool* tool, int& local_count, Value**& local_inits);
	void		add_plugin_local(const MCHAR* var, MSPluginClass* plugin, int& local_count, Value**& local_inits, BOOL constant = FALSE);
...
//	Value*		array_index();
	...
	Value*		function(const MCHAR* name, short flags = 0);
	Value*		function_def();
	Value*		mapped_fn_def();
	Value*		max_command();
	Value*		handler_def();
	...
	Value*		function(const MCHAR* name, Value* owner, short flags = 0);
