...
//	RefResult		NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate) { return REF_SUCCEED; }
	...
	{
		return delegate->GetObjectDisplayRequirement();
	}
	virtual bool PrepareDisplay(
		const MaxSDK::Graphics::UpdateDisplayContext& prepareDisplayContext)
	{
		return delegate->PrepareDisplay(prepareDisplayContext);
	}
	...
	{
		return delegate->UpdatePerNodeItems(updateDisplayContext,
				nodeContext,targetRenderItemContainer);
	}
	virtual bool UpdatePerViewItems(
		...
		MaxSDK::Graphics::IRenderItemContainer& targetRenderItemContainer)
	{
		return delegate->UpdatePerViewItems(updateDisplayContext,
			nodeContext,viewContext,targetRenderItemContainer);
	}
...
#pragma pop_macro("new")
	...
	// From MSPlugin
	virtual HWND			AddRollupPage(HINSTANCE hInst, const MCHAR *dlgTemplate, DLGPROC dlgProc, const MCHAR *title, LPARAM param=0,DWORD flags=0, int category=ROLLUP_CAT_STANDARD);
