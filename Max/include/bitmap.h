...
class BitmapIO : public BaseInterfaceServer {
   ...
   public:
	  ...
	  */
	  ...
	  <b>DWORD piDataSize</b>\n\n
	  The size of the previously saved configuration data.
	  ...
	  * }
	  * \endcode
	  * \param[in] bi The BitmapInfo holding the plugin data
	  * \param[in] iload The ILoad* if called when loading the BitmapInfo from a scene file, NULL otherwise.
	  */
	  BMMExport virtual  BOOL           ValidatePiData              ( BitmapInfo *bi, ILoad *iload = NULL  );
	  ...
	  */
	  BMMExport virtual  BitmapStorage *Load                        ( BitmapInfo *bi, Bitmap *map, BMMRES *status ) = 0;      
		...
		\param bi - The image information.
		\param map - Points to the bitmap to save.
