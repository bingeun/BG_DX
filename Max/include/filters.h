...
class ImageFilterInfo: public MaxHeapOperators {
	  ...
	  const MCHAR    *userlabel;    //-- Optional label given by user
   ...
   public:
	  ...
	  */
	  ...
	  'Negative'). */
	  FLTExport const MCHAR            *UserLabel     ( )         { return userlabel; }

	  //-- Used by VP to update the label. Not to be used by filters.

	  FLTExport void          SetUserLabel   ( const MCHAR *l) { userlabel = l; }
...
private:
	/**
	* This is used internally - the system calls this method when the ImageFilterInfo is loaded. It creates an instance of the
	* ImageFilter and calls ValidatePiData() on that passing in the ImageFilterInfo instance and the ILoad*. The ImageFilter can
	* perform versioning of the plugin data, storing it back on the ImageFilterInfo.
...
class FLT_FilterHandler: public MaxHeapOperators {
   ...
   public:
	  ...
	  void             SetCD              ( ClassDesc *dll )    { cd = dll;}
	  /*! \remarks Returns a pointer to the class descriptor. */
	  ClassDesc       *GetCD              ( )                   { return    cd;}
	  ...
	  <b>DWORD cap</b>\n\n
	  The flags to set. */
	  void             SetCapabilities  ( DWORD cap )      { fltCapability |= cap;}
	  /*! \remarks Returns the capability flags. */
	  DWORD            GetCapabilities  ( )                { return    (fltCapability);}
	  ...
	  BOOL             TestCapabilities ( DWORD cap )      { return    (fltCapability  & cap);}
...
class FLT_FilterList : public MaxSDK::Array<FLT_FilterHandler>
{
private:
	BOOL        listed;
public:
...
/*! \sa  Class ImageFilterInfo, Class Bitmap, Class ITrackViewNode, Class TimeChange, Class UndoNotify, ~{ Working with Bitmaps }~.\n\n
\par Description:
...
class ImageFilter: public MaxHeapOperators {
   ...
   public:
	...
	\endcode
	  ...
	  \par Default Implementation:
	  <b>{ return (FALSE) };</b> */
	  FLTExport virtual  BOOL    LoadConfigure      ( void *ptr, DWORD piDataSize ) { return (FALSE); }
	  ...
	  * }
	  * \endcode
	  * \param[in] fi The ImageFilterInfo holding the plugin data
	  * \param[in] iload The ILoad* if called when loading the ImageFilterInfo from a scene file, NULL otherwise.
	  */
	  FLTExport virtual  BOOL    ValidatePiData     ( ImageFilterInfo *fi, ILoad *iload = NULL  ) { return TRUE; }
	  //-- Preview Facility -------------------------------------------------
	  //
