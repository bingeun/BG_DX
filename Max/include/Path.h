...
public:
    ...
    * e.g. "\\abc\\def" is rooted at backslash.
	...
	/*! \brief Queries whether the file or directory exists. 
	...
	/*!
	 * \brief Creates base file to support incremental upload of Autodesk 360 files if applicable
	 *
	 * This function should be called before a file which is in sync is overwritten with a 
	 * new version (typically during save operation). This is necessary to support incremental 
	 * upload of the file. It must be called every time the file is overwritten (i.e. on every 
	 * save). This function creates a base file using the current version which will be used by 
	 * the sync engine to upload only the changes.
	 * If the file specified by this path is not an Autodesk 360 sync'ed file, no action occurs
	 * as a result of this call.
	 * \return True if a base file was created
	 */
