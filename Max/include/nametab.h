...
class NameTab: public Tab<const MCHAR *> {
	ULONG flags;
	public:
		//! \brief Constructor. 
		...
		/*! \param f - The flag(s) to set. One or more of the following values:			
...
-
		...
		/*! Stores the specified name at the specified position in the list.
			\param i - The position in the list for the name.
			\param n - The name to store. If the name is NULL, the 'i-th' entry is set to NULL. */
		...
		/*! If the new size is smaller than the current size, the entries are deleted.
			\param num - Specifies the size of the list. */
		UtilExport void SetSize(int num);

		//! \brief Removes the 'i-th' name from the list.
		/*! \param i - Specifies the index of the name to remove. */
		...
		/*! \param iload - Provides methods to load data from disk.
			\sa  \ref ioResults. */
		UtilExport IOResult Load(ILoad *iload);

		//! \brief Saves this NameTab to disk.
		/*! \param isave - Provides methods to save data to disk.
			\sa  \ref ioResults. */
